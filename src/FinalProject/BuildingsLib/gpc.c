...
gpc_polygon gpc_create_polygon(int numVerts)
{
    gpc_polygon result;

    result.num_contours = 1;
    
    MALLOC(result.hole, sizeof(int),
        "hole flag array creation", int);
    *result.hole = 0;

    MALLOC(result.contour, sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);

    MALLOC(result.contour[0].vertex, numVerts
        * sizeof(gpc_vertex), "vertex creation", gpc_vertex);

    result.contour[0].num_vertices = numVerts;
